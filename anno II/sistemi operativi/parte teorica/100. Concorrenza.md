...
Un'invariante del problema sarà la presenza di due variabili
```pseudocodice
int nr;  // number readers
int nw;  // number writers
```

usate per arbitrare l'accesso alle risorse, ad esempio nella maniera seguente:
```pseudocodice
(nr >= 0 && nw == 0) || (nr == 0 && nw == 1)
```

Esistono diverse varianti di questo problema, tra le quali una dove si fornisce priorità ai lettori (rischiando starvation per gli scrittori) e viceversa.

Si potrebbero quindi trovare svariate soluzioni, ma quella fornita da Andrews è considerata la più generale ed efficace.
#### Derivazione sistematica della Soluzione (metodo di Andrews)
Anzitutto si crea uno schema di await
```pseudocodice
process Reader {
	< await (nw == 0) -> nr++ >
	read database
...
```
In seguito si trasformano tutte le await in semafori, impiegando la tecnica di **passaggio del testimone** (passing the baton): quando un processo termina, passa la mutua esclusione immediatamente al primo processo che può procedere. Nel caso in cui non ci siano processi idonei, si limita a rilasciare la mutex. Questo avviene mediante un'apposita funzione **signal**.

Quindi ad esempio:
```pseudocodice
< await(B) -> S >  // quando B, esegui S
```
dove:
- **B** è una condizione booleana;
- **S** è uno statement;

richiederebbe l'aggiunta di un semaforo **mutex**, un'array **sem[i]** (per i semafori per ogni condizione) ed un altro array **waiting[i]** (un contatore dei processi in attesa) per essere riscritto nel modo seguente:
```pseudocodice
mutex.P();
...
// esegui S e gestisci la mutua esclusione
S;
signal();
```

Si potrebbe inoltre implementare una variante senza starvation implementando un contatore degli scrittori ```ww``` da controllare prima di lasciare operare i lettori.

### Traendo delle conclusioni - i problemi dei semafori
I semafori sono quindi primitive molto potenti ma anche difficili da comprendere ed implementare correttamente: un errore banale potrebbe causare starvation in un intero sistema, causando enormi danni.

Inoltre la loro natura porta irrimediabilmente a codice disperso e poco strutturato, difficile da mantenere.

Per questo motivo nacquero le **monitor** $\dots$
## Monitor