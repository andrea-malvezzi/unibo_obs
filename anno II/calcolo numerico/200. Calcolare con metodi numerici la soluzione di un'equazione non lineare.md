...
> **N.B.** per tutti gli algoritmi successivi, qualora valesse Weierstrass, un buon modo per scegliere un punto di partenza $x_0$ sarà scegliere il punto medio dell'intervallo su cui si andrà ad operare, ovvero $x_0 = \frac{(a + b)}{2}$. O meglio ancora, il punto in cui $f$ ha valore minore, ma non sempre si conosce tale punto all'inizio dell'algoritmo.
## 2. Metodi Iterativi
...
> **N.B** per funzionare questo algoritmo necessita che la funzione cambi di segni nell'intervallo designato. Per vedere se questo accade o meno, bisogna usare [[200. Calcolare con metodi numerici la soluzione di un'equazione non lineare#1. Teorema di Weierstrass|Weierstrass]]. 
